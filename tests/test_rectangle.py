...
@pytest.mark.parametrize(
    ("side_a", "side_b", "area"),
    [
        pytest.param(10, 7, 70, marks=[pytest.mark.positive], id='integer'),
        pytest.param(5.5, 10.35, 56.92, marks=[pytest.mark.positive], id='float')
    ]
)
def test_rectangle_positive_sides(side_a, side_b, area):
...
@pytest.mark.parametrize(
    ("side_a", "side_b", "area"),
    [
        pytest.param(3.3, 0, 0, marks=[pytest.mark.negative], id='zero'),
        pytest.param(-3, 5, -15, marks=[pytest.mark.negative], id='below zero')
    ]
)
def test_rectangle_negative_int(side_a, side_b, area):
...
@pytest.mark.parametrize(
    ("side_a", "side_b", "area"),
    [
        pytest.param('3', 5, '33333', marks=[pytest.mark.negative], id='int in quotes'),
        pytest.param(15, 'this_is_string', -15, marks=[pytest.mark.negative], id='letter in quotes')
    ]
)
def test_rectangle_string(side_a, side_b, area):
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(8, 11, 50, 7, 10, 438, 188, marks=[pytest.mark.positive], id='add rectangle area'),
        pytest.param(10, 10, 5.5, 2.2, 6.6, 112.1, 143.56, marks=[pytest.mark.positive], id='add square area')
    ]
)
